...
public class Cache implements Element{ // override fn is in the end of this
	...
	public void handleEvent(Event e){ // handle moment
        if(e.getEventType() == Event.EventType.MemoryRead) {
            ...
            if(isHit == true){
                Simulator.getEventQueue().addEvent(
                    new MemoryResponseEvent(Clock.getCurrentTime() + this.latency, this, readE.getRequestingElement(), data)
                );
            } else {
            ...
            Simulator.getEventQueue().addEvent(
				new ExecutionCompleteEvent(
					Clock.getCurrentTime() + Configuration.mainMemoryLatency, containingProcessor.getMainMemory(), writeE.getRequestingElement()
                )
			); 
		} else if(e.getEventType() == Event.EventType.MemoryResponse){
            MemoryResponseEvent E = (MemoryResponseEvent) e;
            cacheWrite(this.msAddr, E.getValue());
        }
	}
}